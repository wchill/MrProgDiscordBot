...
read -p "RabbitMQ admin username: " -r ADMIN_USER
read -p "RabbitMQ admin password: " -r -s ADMIN_PASS
read -p "RabbitMQ worker username: " -r WORKER_USER
read -p "RabbitMQ worker password: " -r -s WORKER_PASS
...
sudo rabbitmqctl add_user "$WORKER_USER" "$WORKER_PASS"
sudo rabbitmqctl set_permissions -p / "$WORKER_USER" ".*" ".*" ".*"
